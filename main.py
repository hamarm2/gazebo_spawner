                ...
                with open(sdfFile, "w") as file:
                    ...
                    for obstacle in obstacles:
                        if(type(obstacle).__name__ == "Box"):
                            wg.box(file, convertX(obstacle.S[0]), convertY(obstacle.S[1]), obstacle.yaw, f"box{boxes:02}",f"box{boxes:02}link", obstacle.a/grid, obstacle.b/grid, obstacleHeight, obstacleDensity)
                            boxes += 1
                        else:
                            wg.cylinder(file, convertX(obstacle.S[0]), convertY(obstacle.S[1]), f"cyl{cyls:02}",f"cyl{cyls:02}link", obstacle.r/grid, obstacleHeight, obstacleDensity)
                            cyls += 1
                    wg.end(file)
